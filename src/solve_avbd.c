...
void s2Solve_AVBD(s2World* world, s2StepContext* context)
{
	...
	for (int i = 0; i < contactCapacity; ++i)
	{
		...
		for (int j = 0; j < contact->manifold.pointCount; j++)
		{
			...
			point->localAnchorA = s2Sub(point->localAnchorA, bodyA->localCenter);
			point->localAnchorB = s2Sub(point->localAnchorB, bodyB->localCenter);

			// Warmstart
			point->normalImpulse = ALPHA * GAMMA * point->normalImpulse;
			point->tangentImpulse = ALPHA * GAMMA * point->tangentImpulse;
			point->penalty = S2_CLAMP((GAMMA * point->penalty), PENALTY_MIN, PENALTY_MAX);
	...
	for (int it = 0; it < context->iterations; ++it)
	{
		...
		for (int bi = 0; bi < bodyCapacity; ++bi)
		{
			...
			for (int i = 0; i < contactCapacity; ++i)
			{
				...
				for (int j = 0; j < contact->manifold.pointCount; j++)
				{
					...
					s2Vec2 F = s2Add(s2MakeVec2(point->normalImpulse, point->tangentImpulse), s2MulSV(point->penalty, C));
					...
					lhs = s2AddScaledOuter(lhs, point->penalty, J1);
					lhs = s2AddScaledOuter(lhs, point->penalty, J2);
				}
			}
		...
		for (int i = 0; i < contactCapacity; ++i)
		{
			...
			for (int j = 0; j < contact->manifold.pointCount; j++)
			{
				...
				if (F.x < 0)
					point->penalty = S2_MIN((point->penalty + fabsf(C.x) * BETA), PENALTY_MAX);
			}
		}
		...
		for (int i = 0; i < jointCapacity; ++i)
		{
			...
			if (joint->type == s2_revoluteJoint)
			{
				...
				joint->revoluteJoint.impulse = s2ClampSV(s2Add(joint->revoluteJoint.impulse, s2Mul(joint->revoluteJoint.penalty, C)), -LAMBDA_MAX, LAMBDA_MAX);
			...
			else if (joint->type == s2_mouseJoint)
			{
				...
				joint->mouseJoint.impulse = s2ClampSV(s2Add(joint->mouseJoint.impulse, s2Mul(joint->mouseJoint.penalty, C)), -LAMBDA_MAX, LAMBDA_MAX);
